...
reader.each do |node|
  ...
  items << {}
  items[-1]['description'] = item['desc'][0].gsub(/\n/, ' ').gsub(/\s+/, ' ').strip
  items[-1]['name'] = item['name']
  items[-1]['published'] = nil
  items[-1]['updated'] = nil
  if item['phase']
    items[-1]['published'] = Time.parse("#{item['phase'][0]['date']} 00:00:00.000000000 Z")
    items[-1]['updated'] = Time.parse("#{item['phase'][0]['date']} 00:00:00.000000000 Z")
  end
end
